...
'Component Count' is the number of elements in the corresponding dimension of
 ...
 'Component Count' must be a 'Result <id>' of an *OpConstant*, *OpSpecConstant*,
 'Component Count' must be a 'Result <id>' of an *OpConstant*, *OpSpecConstant*,
 <<DebugGlobalVariable,*DebugGlobalVariable*>>, or
 <<DebugLocalVariable,*DebugLocalVariable*>>. If it is an *OpConstant* or
 *OpSpecConstant*, its type must be a 32-bit or 64-bit integer type. Otherwise
 its type must be a <<DebugTypeBasic,*DebugTypeBasic*>> whose 'Size' is 32 or 64
 and whose 'Encoding' is *Unsigned*. If the *OpConstant* or *OpSpecConstant*
 value is set to 0, this indicates an array with an unknown size at compile time
 which is sized at runtime, corresponding to the SPIR-V *OpTypeRuntimeArray* type. +
 <<DebugLocalVariable,*DebugLocalVariable*>>. If it is an *OpConstant* or
 *OpSpecConstant*, its type must be a 32-bit or 64-bit integer type. Otherwise
 its type must be a <<DebugTypeBasic,*DebugTypeBasic*>> whose 'Size' is 32 or 64
 and whose 'Encoding' is *Unsigned*. If the *OpConstant* or *OpSpecConstant*
...
'Vector Type' is the '<id>' of a debugging instruction that describes the type of
 ...
 +
'Vector Count' is the '<id>' of a 32-bit integer *OpConstant* or *OpSpecConstant*
 denoting the number of vectors in the matrix. +
'Vector Count' is the '<id>' of a 32-bit integer *OpConstant* or *OpSpecConstant*
 denoting the number of vectors in the matrix. +
 +
