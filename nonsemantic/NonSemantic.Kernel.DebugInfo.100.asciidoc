SPIR-V NonSemantic Kernel DebugInfo Instructions
================================================
...
Introduction
------------
This is the specification of the *NonSemantic.Kernel.DebugInfo.100* extended instruction
set.

This extended instruction set is imported into a SPIR-V module in the following
manner:

[[OpExtInstImport]]`<extinst-id> OpExtInstImport "NonSemantic.Kernel.DebugInfo.100"`
...
(transferred from NonSemantic.Shader.DebugInfo.100):
...
* New instructions: <<DebugSourceContinued,*DebugSourceContinued*>>,
  ...
  <<DebugStoragePath,*DebugStoragePath*>>, <<DebugEntryPoint,*DebugEntryPoint*>> and
...
Additional list of changes to the NonSemantic.Shader.DebugInfo.100 specification:
...
| 109 | <<DebugModule,*DebugModule*>>
| 110 | <<DebugTypeSubrange,*DebugTypeSubrange*>>
| 111 | <<DebugTypeArrayDynamic,*DebugTypeArrayDynamic*>>
| 112 | <<DebugTypeString,*DebugTypeString*>>
|======
...
10+|[[DebugSource]]*DebugSource* +
 ...
 path. +
 +
 '<<ChecksumKind,Checksum Kind>>' is the '<id>' of a 32-bit integer *OpConstant*.
 It represent type of algorithm used to calculate 'Checksum'. +
 +
 'Checksum' is the '<id>' of *OpString* that contains checksum of the source file. +
 +
...
'Data Location' is either a 'Result <id>' of <<DebugExpression,*DebugExpression*>>
 instruction that's used as a descriptor to calculate the address of the actual
 data or a 'Result <id>' or <<DebugInfoNone,*DebugInfoNone*>> instruction.
 If the latter is applied, then the address of the data is the same as the
 address of the object. +
 +
'Associated' is an *OpConstant* with 32-bit or 64-bit integer type. A non-zero
 value indicates that the dynamic array is dynamically associated with other
 objects. +
 +
'Allocated' is an *OpConstant* with 32-bit or 64-bit integer type. A non-zero
 value indicates that the dynamic array is allocatable and deallocatable. +
 +
'Rank' is an *OpConstant* with 32-bit or 64-bit integer type and its value
 specifies the rank of the dynamic array. +
 +
'Subranges' is the elements in the corresponding dimension of the array. The
 number and order of 'Subranges' operands must match with the number and order of
 array dimensions as they appear in the source program. 'Subranges' must be a
...
[cols="2*1,3*2,1,5*3"]
|======
11+|[[DebugTypeString]]*DebugTypeString* +
 +
...
'Base Type' is a debugging instruction that describes how each character is
 encoded and is to be interpreted. +
 +
'Data Location' is either a 'Result <id>' of <<DebugExpression,*DebugExpression*>>
 instruction that's used as a descriptor to calculate the address of the actual
 data or a 'Result <id>' or <<DebugInfoNone,*DebugInfoNone*>> instruction.
...
| 10+ | 13 | '<id>' +
'Result Type' | 'Result <id>' | '<id> Set'| 112
| '<id> Base Type'
| '<id> Data Location'
| '<id> Size'
| '<id> LengthAddr'
...
|1.00 Rev 1  |2023-02-10|Alexey Sotkin / Dmitry Sidorov     |*Initial revision*
|===============================================================================
