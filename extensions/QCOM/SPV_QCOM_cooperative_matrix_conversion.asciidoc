...
IEEE 754 encoding (32-bit or 16-bit), its _Rows_ must be equal to SubgroupSize and _Columns_ must be 8,
16, 32 and 32 when its Component Type is the 32-bit OpTypeFloat, the 16-bit OpTypeFloat, the signed
8-bit OpTypeInt, and the unsigned 8-bit OpTypeInt, respectively.
...
respectively and its _Columns_ must be one of SubgroupSize, SubgroupSize/2, and SubgroupSize/4
(e.g., for SubgroupSize == 64, _Columns_ is one of 64, 32 and 16).

The _Source Array_'s _Length_ must be 8 if its _Element Type_ is unsigned 32-bit OpTypeInt and
must match the _Result Type_'s _Rows_ otherwise.

The array from invocation with SubgroupLocalInvocationID _i_ goes to the _i_-th column of the resulting cooperative matrix.
If the _Result Type_'s Columns is SubgroupSize/2, the values from invocations [SubgroupSize/2 .. SubgroupSize) are ignored.
If the _Result Type_'s Columns is SubgroupSize/4, the values from invocations [SubgroupSize/4 .. SubgroupSize) are ignored.
...
IEEE 754 encoding (32-bit or 16-bit), its _Rows_ must be equal to SubgroupSize and _Columns_ must be a
power of 2 which is greater than or equal to 16 and less than or equal to SubgroupSize (e.g., for
SubgroupSize == 64, _Columns_ is one of 64, 32, and 16).
...
with the default IEEE 754 encoding (32-bit or 16-bit).
...
Invocation with SubgroupLocalInvocationID _i_ gets the _i_-th column of the cooperative matrix.
If the _Source CoopMat_'s Columns is SubgroupSize/2, invocations [SubgroupSize/2 .. SubgroupSize)
get an OpUndef value of the _Result Type_.  If the _Source CoopMat_'s Columns is SubgroupSize/4,
invocations [SubgroupSize/4 .. SubgroupSize) get an OpUndef value of the _Result Type_.
