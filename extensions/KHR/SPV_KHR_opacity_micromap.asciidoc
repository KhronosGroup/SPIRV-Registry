...
* Declaring an OpExtension string in a module does not, by itself,
  determine whether opacity micromap data is used during ray query
  traversal. That is determined by which Vulkan opacity_micromap
  extension is enabled at the API level:
  ...
  ** With only VK_KHR_opacity_micromap enabled and its `micromap`
     feature enabled, opacity micromap data present in the
     acceleration structure is used during ray query traversal only
     when the module declares the OpacityMicromapIdKHR execution mode
     with its specialization-constant operand equal to VK_TRUE.

  This execution mode does not affect traversal initiated by pipeline
  trace ray instructions, which is governed separately by the Vulkan
  API, however it does apply to ray queries issued from a shader in a ray
  tracing pipeline.

* If ray query traversal uses an acceleration structure that contains
  opacity micromap data with only VK_KHR_opacity_micromap enabled and
  its `micromap` feature enabled, but the module does not declare the
...
|5 |2026-09-10 |Daniel Koch    | Scope execution mode wording to ray query traversal (#2810)
|========================================
