...
Must be applied only to an *OpFunctionParameter* of a function that is an *entry point*. Indicates that this input will not change during the execution of piplined kernel invocations. Input can change once all active invocations have finished.
2+| | *FPGAArgumentInterfacesINTEL*
|====
--
...
It is invalid to specify multiple copies of the same decoration on the same *OpFunctionParameter*. For example, you can't specify two *MMHostInterfaceAddressWidthINTEL* decorations on the same *OpFunctionParameter* with conflicting values.
