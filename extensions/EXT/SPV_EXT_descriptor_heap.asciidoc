...
Change the following paragraph in *OpUntypedArrayLengthKHR* from:
...
> The type of Pointer must be a pointer type. Pointer must have the same value as a storage buffer descriptor. That is, the value must be one of:
>
>  * the same as the result of *OpBufferPointerEXT* in the StorageBuffer storage class
>  * the same as a variable decorated with DescriptorSet, Binding, and Block in the StorageBuffer storage class
>  * the same as an element of one of the above when the data type is an array of structures
>  * the same as a variable in the PhysicalStorageBuffer storage class
