...
[cols="7*1"]
|======
6+|[[OpHitObjectRecordFromQueryEXT]]*OpHitObjectRecordFromQueryEXT* +
 +
 ...
 +
 'Hit Object Attributes' contains the attributes of the hit which are to be encoded in 'Hit Object'. This must be an OpVariable in the *HitObjectAttributeEXT* storage class. If the committed intersection in 'Ray Query' is *RayQueryCommittedIntersectionTriangleKHR*, the value of 'Hit Object Attributes' is ignored, and the hit object attributes are taken from the committed triangle intersection encoded in 'Ray Query'. If the committed intersection in 'Ray Query' is *RayQueryCommittedIntersectionAABBKHR*, the value of 'Hit Object Attributes' is used as the source of the hit object attributes to be encoded in 'Hit Object'. +
 +
 'Hit Kind' is optional. If present, it is the integer hit kind to be encoded in 'Hit Object'. It must be a 32-bit 'unsigned integer type' scalar. If the committed intersection in 'Ray Query' is *RayQueryCommittedIntersectionAABBKHR*, 'Hit Kind' must be present. If the committed intersection in 'Ray Query' is *RayQueryCommittedIntersectionTriangleKHR*, the value of 'Hit Kind' is ignored, and the hit kind is taken from the committed triangle intersection encoded in 'Ray Query'. +
 +
 This instruction is allowed only in *RayGenerationKHR*, *ClosestHitKHR* and *MissKHR* execution models. +
 +
1+|Capability: +
...
1) Must 'Hit Kind' be provided for AABB intersections in *OpHitObjectRecordFromQueryEXT*?
+
--
*RESOLVED*: Yes. Revision 2 and earlier of this specification erroneously did
not provide a hit kind for AABB intersections. For compatibility with SPIR-V
generated against those revisions, implementations in practice should accept
*OpHitObjectRecordFromQueryEXT* for AABB intersections even when 'Hit Kind' is
not provided.
--
